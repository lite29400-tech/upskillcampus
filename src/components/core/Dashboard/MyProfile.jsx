...
export default function MyProfile() {
  ...
  return (
    <>
      ...
      <div className="my-10 flex flex-col gap-y-10 rounded-2xl border-[1px] border-richblack-700 bg-richblack-800 p-8 px-7 sm:px-12">
        <div className="flex w-full items-center justify-between">
          <p className="text-lg font-semibold text-richblack-5">Earned Badges 🏆</p>
        </div>
        <div className="flex gap-4 flex-wrap">
          {user?.badges?.length > 0 ? (
            user.badges.map((badge, index) => (
              <div key={index} className="flex items-center gap-2 rounded-full bg-yellow-50 px-3 py-1 text-richblack-900 font-bold">
                <span>🏅</span>
                <span>{badge.type || badge}</span>
              </div>
            ))
          ) : (
            <p className="text-richblack-300">Complete courses to earn badges!</p>
          )}
        </div>
      </div>
    </>
